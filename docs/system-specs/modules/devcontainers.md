...
- Warm-pool sessions are not containerized: pooled providers are pre-spawned with
  `default_project_dir()` as cwd before any project is known, so a claimed pooled
...
| `acp/client.py` | `_maybe_devcontainer_info()` (eligibility), the spawn branch that replaces argv, `_acp_cwd()`, in-container kill on teardown. |
