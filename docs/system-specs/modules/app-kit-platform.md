...
`platform_compat.is_dir_link` and removal done with
`platform_compat.unlink_dir_link`. Two failure modes follow from getting this
...
`reconcile_app_skills`. Shim: `platform_compat.symlink_dir` / `is_dir_link` /
`unlink_dir_link`.
