...
`src/kiro_crew/static/dist` under the target checkout's OWN venv interpreter
(same reasoning as the pip step: its editable install decides which checkout is
staged). It is not cosmetic. On a source install `static/dist` is a *symlink*
