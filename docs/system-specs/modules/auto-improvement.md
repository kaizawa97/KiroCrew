...
fix it". So the shell stays, the **forge CLIs are allowlisted** (`gh`/`glab` may run only the
read-only verbs the prompts name — `pr view`, `pr checks`, `pr diff`, `pr list`, `run view`,
`issue view`, `repo view` and their siblings; every other verb is refused, including the ones
a denylist kept missing: `gh alias set`, `gh repo delete`/`edit`, `gh gist create`,
`gh ssh-key add`, `gh variable set`, `gh pr lock`), and the remaining **verbs are denied**:
`git push`/`send-pack` (under their dashed `git-push` spellings too), `git remote
set-url`, any `git -c`/`--config-env`/`git config`/`GIT_CONFIG_KEY_*` that sets
`alias.*`, `credential.*`, `http.*`, `url.*`, `remote.*`, `protocol.ext.*` or a key whose
LEAF names a program git runs (`core.fsmonitor`, `core.hooksPath`, `diff.<d>.command`,
`merge.<d>.driver`, `gpg.ssh.program`, …), `GIT_CONFIG*` variables pointing at another
config file, `curl`/`wget`, `ssh`/`scp` — plus `eval`, the assignment builtins and
`env -S`/`--split-string`, whose arguments are re-analyzed as the nested commands they
are. The repo's own `is_sensitive_bash_command` was checked first and does not cover
this — it allows `gh pr merge`.

**What that leaves un-confined, stated plainly.** The shell gate reads the command a request
ASKS for, not what that command then does, so it is a first barrier and not a boundary — and
it cannot become one. `bash fix.sh` and `python3 helper.py` name a file this same agent's
`Write` tool authored; no matcher over a command LINE can read that file, and an alias the
agent wrote into the clone's own `.git/config` renames a verb with no option for the gate to
see. Closing spellings one at a time narrows an unbounded set by one each time, which is the
review smell `AGENTS.md` names. So the boundary is placed where it is enforced instead:

* **Credentials are confined by a REFUSAL, not by the shell gate.** The earlier text here
  measured `sandboxed_spawn_argv(mode="strict")` and concluded a nested process sees
  `~/.aws`, `~/.config/gh` and `~/.docker` empty and `gh auth status` reports not-logged-in.
  The measurement was real and it described the **wrong path**: `strict` is the `claude -p`
  subprocess spawn, which `_make_runner` refuses. Production watchers take
  `SessionAgentRunner` → `create_provider_factory` → `wrap_argv(mode=agent.sandbox)`, and the
  shipped `auto`/`standard` tier's `_STANDARD_DIRS` does not list `.config/gh`. So the
  operator's GitHub OAuth credential was inside the watcher's sandbox by construction.
  `_make_runner` now runs the loop's own `_credentials_are_unconfined()` gate and refuses to
  build a watcher runner at all while the effective tier leaves that store readable, and it
  passes an `extra_env` that empties `GH_TOKEN`/`GITHUB_TOKEN`/`GH_ENTERPRISE_TOKEN`/
  `GITHUB_ENTERPRISE_TOKEN` for the session on every path (`strip_credential_env` runs only in
  the subprocess spawn, so the provider path had no env scrub either). D-143.
...
gap. Those tests pin the WATCHER'S EFFECTIVE TIER, not the contents of `_STRICT_DIRS`: an
assertion about a tier the production path never selects is how the false claim above
...
| D-143 | The watcher's credential claim was measured on a path production never takes | D-73 said "a watcher cannot publish", D-84 declined removing `Bash` because the escalation is "blocked one layer down … enforced rather than pattern-matched" by `strip_credential_env` + `sandboxed_spawn_argv(mode="strict")` hiding `~/.config/gh`, and the `security_posture` disclosure the operator reads before setting `watcherAcceptEgressRisk` said "credential stores are hidden by the strict sandbox (verified)". The measurements were real; they described the **`claude -p` subprocess path, which `_make_runner` REFUSES** (D-118's own fallback removal). The only production path is `SessionAgentRunner` → `create_provider_factory` → `AcpProvider(sandbox_mode=agent.sandbox)` → `wrap_argv`, whose shipped `auto`/`standard` tier omits `.config/gh` from `_STANDARD_DIRS` — and `agent.sandbox`'s enum offers only `auto`/`off`, so no documented setting hid it. `strip_credential_env` likewise runs only in the refused spawn, so a gateway `GH_TOKEN` was inherited too. Net effect: an unattended turn whose prompt embeds outsider-written PR text held the operator's GitHub OAuth credential, with `shell_command_refusal` — the spec's own "first barrier and not a boundary" — as the only publish control, and that denylist passed `bash fix.sh` (a file the allowed `Write` tool authored, no `-c`, so every branch fell through), `git -c alias.p=push p <url> HEAD:main` (`-c` is value-taking, so `alias.p=push` was skipped and `('p',)` never equalled `('push',)`), `python3 helper.py`, and every `gh` write verb nobody had listed (`alias set`, `repo delete`/`edit`, `gist create`, `ssh-key add`, `variable set`, `pr lock`), while `glab` had no entry at all. FIXED at the boundary rather than by another spelling: `_make_runner` runs the loop's own `_credentials_are_unconfined()` — shared, not copied, so the two unattended runners cannot disagree — and refuses to build a watcher runner while the effective tier leaves the store readable, re-asserts BOTH hard gates before every pass (a runner is built once per watcher and then drives up to `max_nudges` turns over hours, so a build-time-only check let a watcher started while opted in outlive the revocation; the six pre-existing `TestNudgeLoop` tests that drive the loop with no injected factory now seed both flags, found by review round two), and passes an `extra_env` that empties the four GitHub token variables for the session on every path. The admitted tier set is `{'strict'}` alone: `.config/gh` occurs exactly ONCE in `sandbox.py`, inside `_STRICT_DIRS`, so the inherited `{'cc','strict'}` would have cleared the gate on a host whose governed floor is `cc` while `hosts.yml` stayed readable — found by the adversarial review of this change, and the test written to pin the set was RED against it. That test asserts against `_sandbox_policy().strict_dirs()`/`cc_dirs()`, the CPP extension points the builders actually read, not the module globals: a companion returning a REPLACEMENT list would otherwise keep it green while the gate cleared an unmasked tier. That helper was also reading a non-existent top-level `KiroCrewConfig.sandbox`, so it reported every gateway "unset" and never consulted a tier; it now asks `sandbox.configured_sandbox_mode` / `effective_sandbox_mode` (honouring a governed `sandbox.min_level` floor) and `credential_mask_applies` (several `wrap_argv` paths hand back an UNWRAPPED child, where the tier name says confined and no mask lands). Hardened alongside, because they cost nothing and are enumerable: `gh`/`glab` moved to an ALLOWLIST of read-only verbs, `git send-pack` joined `push`, dashed builtins (`git-push`, 166 of which ship in git 2.43's exec-path) are rewritten to the `git <verb>` form so one table judges both spellings, `eval` and the assignment builtins (`export`/`declare`/`typeset`/`readonly`) joined the wrapper table, `-c`/`--config-env`/`git config`/`GIT_CONFIG_KEY_*`/`GIT_CONFIG_PARAMETERS` assignments to `alias.*`, `credential.*`, `http.*`, `url.*`, `remote.*`, `protocol.allow`/`protocol.ext.*`, `include*` or a key whose LEAF names a program git runs are refused, `GIT_CONFIG`/`GIT_CONFIG_GLOBAL`/`GIT_CONFIG_SYSTEM` are refused whatever their value (they name a file the agent's own `Write` tool can author), and `env -S`/`--split-string` is re-analyzed as the nested command it is. Those came from TWO adversarial review rounds on this very change and were each MEASURED first: `env -S'git push'` ran the push because the option-stripping read the verb as an option value; `stdbuf -o0 glab mr merge 3` was allowed because the option-value-consumption guard consulted the two DENY tables and not the new ALLOW one — and round two found the same guard missing `_SHELL_BINARIES`, which subsumes it, so `env -i sh -c 'git push'`, `stdbuf -o0 bash -c 'curl …'` and `xargs -I{} sh -c '…'` were all open (a valueless `-i`/`-o0`/`-I{}` ate the shell, then its own `-c` was read as a value-taking option and swallowed the script); `eval "git push"` and `export GIT_CONFIG_KEY_0=alias.p; git p` were allowed because neither `eval` nor the assignment builtins were wrappers; and a prefix table closed `diff.external` while leaving `diff.d.command`, `core.alternateRefsCommand`, `interactive.diffFilter`, `pager.<cmd>`, `submodule.<n>.url` and `init.templateDir` open, because that family is not prefix-shaped. The guard now consults every table that judges a binary, and the config family is matched by LEAF plus a `pager.` prefix that leaves `core.pager` alone. The LEAF rule deliberately excludes `pager` — `git -c core.pager=cat diff` is diff reading — and `protocol.` was narrowed to `protocol.allow`/`protocol.ext.` after the blanket prefix was measured refusing `protocol.version=2` and the documented CVE-2022-39253 workaround `protocol.file.allow=always`. DELIBERATELY NOT hardened: shells and interpreters running a script FILE. The subject is wrong for a command-line matcher — the file is not in the command — and an alias written into the clone's `.git/config` renames a verb with nothing for the gate to see, so both are stated as residual instead of chased. Docs and the posture disclosure now describe the boundary enforced on the `SessionAgentRunner` path, and the tests pin the WATCHER'S EFFECTIVE TIER rather than the contents of `_STRICT_DIRS` — an assertion about a tier the production path never selects is how the false claim survived four review rounds. |
