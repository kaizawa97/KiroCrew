...
- Sessions served from the warm pool are not containerized. Pooled processes are
  pre-spawned before any project is known, with the default workspace directory
  as their cwd, so a session that claims a pooled provider runs on the host even
  when its project would otherwise qualify. Set `session.pool_size` to `0` if
  every session must be containerized.
