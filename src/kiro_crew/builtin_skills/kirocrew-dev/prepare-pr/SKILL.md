...
- **Readiness signal**: GitHub shows exactly one managed label: `readiness: checking`, `readiness: action required`, `readiness: maintainer review`, or `readiness: passed`. For same-repository PRs, the matching `PR Readiness` commit status aggregates CI, Build, Code Review, all three GPT passes, Claude, Design, CodeQL, and Arbiter for the current SHA. Fork PRs cannot receive the repository's secret-backed AI reviews, and this repository's managed CodeQL workflow is not scheduled for fork heads; because passing public CI/Build/Code Review is therefore NOT full validation, a fork PR never reaches `passed` — it terminates at a dedicated RED `readiness: maintainer review` (`PR Readiness` = failure) so a maintainer cannot accidentally merge code the AI reviewers never saw, and must review manually or re-run validation from a trusted in-repo branch first. `passed` means all eligible automated validation passed; live mergeability, behind-base state, human review, and branch protection remain separate gates checked by `pr_status.py` and GitHub.
...
The loop is project-agnostic; everything project-specific — the **local gates**, the **reviewers**, and the **conventions** (single-commit rule, base branch, the aggregate-readiness status name, the long-term-defer label) — comes from a resolved **project profile**, not hardcoded in this prose. Resolve it once per run and keep the JSON for Phases 1–3:
...
4. **Poll** `python3 $SKILL_DIR/scripts/pr_status.py <pr#>`:
