...
_CREW_HOME = (
    Path(_CREW_HOME_ENV).expanduser()
    if _CREW_HOME_ENV
    else Path(_HOME_REAL) / ".kiro" / "crew"
)
...
def _start_inject_proxy(dev_url: str) -> tuple[object | None, str]:
    ...
    bound = cast(
        ...
        type(
            "_BoundDevProxy", (_DevProxyHandler,), {"upstream_host": host, "upstream_port": port}
        ),
    )
    try:
...
class _StaticInjectHandler(BaseHTTPRequestHandler):
    ...
    def _dispatch(self) -> None:
        ...
        status, ctype, body = _static_response(
            str(root), rest, f"/{first}/", script=_OVERLAY_PATH
        )
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _h_thread(self, qs: dict) -> None:
        ...
        def _txn() -> tuple[int, dict]:
            ...
            if cid:
                target = next(
                    (c for c in (req.get("comments") or []) if c.get("cid") == cid), None
                )
                if target is None:
