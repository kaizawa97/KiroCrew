...
def classify(record: dict, config: dict | None = None) -> dict:
    ...
    if override in ("red", "yellow", "green") and override != band:
        ...
        reason = pipeline._redact(
            (p1.get("band_override_reason") or "").strip()) or "AI judgment"
...
# `band` is the ONLY row field exempt from redaction, and the only one that earns
# it: `classify()` computes it, `band_override` can only set it to one of
# red/yellow/green, and both sides index by its exact value
# (`bands[row["band"]]` here, `BAND_DOT[row.band]` in ReportView).
#
# The earlier version of this set also exempted `change_id`, `platform`,
# `gate_verdict`, `blast` and `design_risk` on the claim that they are "validated
# against fixed vocabularies". That was wrong: `validate_result` enforces a
# vocabulary for `gate_verdict` alone, so the rest are worker-authored strings
# that were being handed a redaction exemption. Redaction is shape-based and
# leaves every legitimate value of these fields byte-identical (verified for
# `GH-owner-repo-N`, github/gitlab, PASS/CONCERNS/BLOCK, SMALL/MEDIUM/LARGE,
# low/medium/high), so scrubbing them costs nothing and closes the exemption.
_STRUCTURAL_ROW_FIELDS = frozenset({"band"})


def _redact_row(row: dict) -> dict:
    """Scrub every string in a report row except `band`, at any depth.
    ...
    while the prose fields beside it were scrubbed. The set to skip is enumerated
    rather than the set to scrub: a field added to the row later is redacted by
    ...
    """
    return _redact_deep_map(row, frozenset(_STRUCTURAL_ROW_FIELDS))
...
def read_report(root: Path | None = None, run_id: str | None = None) -> dict | None:
    ...
    return {
        "bands": report.get("bands") or {"red": 0, "yellow": 0, "green": 0},
        "rows": rows,
        "generated_at": report.get("generated_at") or idx.get("generated_at") or "",
        "total": idx.get("total", len(rows)),
        "report_slug": idx.get("report_slug"),
    }
