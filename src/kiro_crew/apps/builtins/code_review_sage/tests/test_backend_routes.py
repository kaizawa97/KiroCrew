...
class TestNestedStringFieldsMustBeScalars:
    ...
    `phase1` is checked by SHAPE — every value must be a scalar — because naming
    the fields is exactly what let this recur twice. `blast_radius` cannot take
    that rule: `signals` is legitimately a nested object in the schema.
    """
    ...
    def test_a_non_scalar_phase1_value_is_rejected(self, field, bad):
        ...
        assert any(f"phase1.{field} must be a scalar" in e for e in errs), errs
        ...
        for bad in bad_verdicts:
            errs = results.validate_result(self._record(
                phase1={"gate_verdict": bad, "design_risk": "low",
                        "criticality": "low"}))
            assert any("phase1.gate_verdict must be a scalar" in e for e in errs), errs
    ...
    def test_write_result_refuses_the_record(self, tmp_path):
        ...
        store.ensure_layout(tmp_path)
        with pytest.raises(ValueError, match="phase1.design_risk must be a scalar"):
            results.write_result(
