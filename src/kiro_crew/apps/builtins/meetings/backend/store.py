...
# A note image's name is always generated by the server: uuid4 hex + a sniffed
# extension. Anchored and exhaustive so nothing else can be a note-image path.
_SAFE_NOTE_IMAGE_RE = re.compile(r"^[0-9a-f]{32}\.(?:png|jpg|gif|webp)$")
...
def note_images_dir(meeting_id: str, root: Path | None = None) -> Path:
    """Directory holding images pasted into a meeting's note, containment-checked.

    A subdirectory rather than the meeting dir itself: agent output files are flat
    files there, so keeping images one level down means an image can never share a
    name with one, whatever a future widget extension turns out to be.
    """
    return contain(
        meeting_dir(meeting_id, root) / k.NOTE_IMAGES_DIR,
        operation="meetings.note_images",
        root=root,
    )


def safe_note_image_name(raw: object) -> str:
    """Validate a note-image filename: 32 hex characters plus a known extension.

    Exactly the shape ``handle_post_note_image`` generates, and nothing else.
    ``contain`` alone is NOT sufficient here and it is worth saying why: it checks
    containment under the meetings DATA ROOT, so ``../escape.png`` resolves to
    ``<data>/meetings/escape.png`` and passes — as would ``../m2/note-taker.md``,
    which is another meeting's AGENT OUTPUT file. Nothing currently hands a client
    string to :func:`note_image_path`, but "nothing currently does" is not a
    barrier, and this is.
    """
    if not isinstance(raw, str):
        raise MeetingsPathError("image name must be a string")
    if not _SAFE_NOTE_IMAGE_RE.match(raw):
        _audit("meetings.note_image_name", str(raw)[:120], outcome="denied")
        raise MeetingsPathError("image name is not a generated note-image name")
    return raw


def note_image_path(meeting_id: str, filename: str, root: Path | None = None) -> Path:
    """Path of one note image, name-validated and containment-checked.

    Two barriers, because they stop different things: the NAME check keeps the write
    inside this meeting's own directory, and ``contain`` keeps it inside the data
    root at all (and resolves symlinks while doing so).
    """
    return contain(
        note_images_dir(meeting_id, root) / safe_note_image_name(filename),
        operation="meetings.note_image",
        root=root,
    )


def write_note_image(meeting_id: str, filename: str, data: bytes, root: Path | None = None) -> Path:
    """Write one note image and return its path. BLOCKING."""
    path = note_image_path(meeting_id, filename, root)
    path.parent.mkdir(parents=True, exist_ok=True)
...
def read_note(meeting_id: str, root: Path | None = None) -> dict[str, Any]:
    ...
    # the note's RELATIVE image links (``images/xxx.png``) against it — that is what
