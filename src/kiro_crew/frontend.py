...
_BUILD_TIMEOUT = 300
# Seconds to wait for a SIGKILLed build tree to be reaped before giving up on
# draining its pipes. SIGKILL is not catchable, so this only covers the kernel
...
def _staging_lock(static_parent: Path) -> Iterator[None]:
    ...
    Serializes every KiroCrew-initiated build or stage of the frontend: Dev
    Fleet's
    Pull+Build and the dashboard update flow can run at once, and BOTH the
    ``npm run build`` (which empties ``website/dist``) and the copy/swap must be
    inside one holder. Covering only the copy still lets a peer's build rewrite
...
def build_and_stage(
    ...
    ``npm`` names the executable to run, so a caller that resolved a trusted
    path passes it rather than having it re-resolved here. Returns ``True``
    when ``static/dist`` holds the newly built bundle.
    """
    root = proj_path or Path(__file__).resolve().parents[2]
...
def _stage_dist(
    ...
    self-contained snapshot independent of later ``website/`` rebuilds. That
    independence is load-bearing for a *running* gateway: aiohttp resolves a
    static route's directory once at registration, so a gateway started while
    ``static/dist`` was a symlink (see ``ensure_dev_dist_symlink``) is pinned to
    ``website/dist`` for its whole life and 404s while Vite rewrites that
    directory. Staging makes the NEXT start serve an independent tree.
    ...
    Returns ``True`` when ``static/dist`` holds the staged build.
    """
    static_dist = proj_path / "src" / "kiro_crew" / "static" / "dist"
...
def stage_built_dist(
    ...
    """Stage a built frontend into this install's ``static/dist``.

    For callers that stage a build they did not run themselves. A caller that
    also runs the build must use ``build_and_stage`` instead, so one lock holder
    spans both.

    ``proj_path`` defaults to the repo root inferred from this module, so the
    invoked interpreter's own editable install decides which checkout is
    staged. ``built_dist`` defaults to that checkout's ``website/dist`` and can
    name a scratch build directory instead.

    A source that is not a complete build — no ``index.html``, or an
    ``index.html`` whose ``/assets/`` chunks are absent — is refused rather than
    published, since an out-of-band build (one that takes no staging lock, such
    as pod provisioning) can be observed mid-rebuild. Returns ``True`` on
    success.
    """
    root = proj_path or Path(__file__).resolve().parents[2]
    return _stage_dist(built_dist or root / _DIR_NAME / "dist", root)
...
def build_frontend_sync(
    ...
    try:
        with _staging_lock(proj_path / "src" / "kiro_crew" / "static"):
            _npm_build_and_stage_locked(website_dir, proj_path, npm_bin, log)
    except OSError as exc:
        log(f"  ⚠️  Could not acquire the static/dist staging lock: {exc}")
        ...
        try:
            with _staging_lock(proj_path / "src" / "kiro_crew" / "static"):
                return _npm_build_and_stage_locked(
                    website_dir, proj_path, npm_bin, messages.append
                )
        except OSError as exc:
