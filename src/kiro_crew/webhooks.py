...
class WebhookTokenStore:
    ...
    def _load(self) -> list[dict[str, Any]]:
        ...
        if not isinstance(entries, list):
            return []
        return [e for e in entries if isinstance(e, dict) and e.get("token_hash")]
...
class WebhookRunStore:
    ...
    def _load(self) -> list[dict[str, Any]]:
        ...
        if not isinstance(runs, list):
            return []
        # Drop rows whose outcome is not one this build knows. record() validates
        # on write, but the file is also whatever is on disk after a hand-edit, a
        # partial write, or a downgrade from a build that had more outcomes. The
        # dashboard indexes its label/badge table by outcome, so an unknown value
        # reaches the client as a missing entry and takes the whole page down with
        # its error boundary. Filtering here keeps that to one skipped row.
        return [
            r for r in runs
            if isinstance(r, dict) and r.get("outcome") in VALID_OUTCOMES
        ]
