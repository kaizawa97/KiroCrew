...
def config_digest(config_path: Path) -> str:
    ...
    content per regular file, symlinks skipped), not just devcontainer.json:
    a referenced Dockerfile, compose file, or postCreateCommand script can
    change what a build executes while the json stays byte-identical (M3
    review finding). For the root-layout ``.devcontainer.json`` there is no
    directory; the digest covers that one file, read through the same
    hardened opener.
    """
    h = hashlib.sha256()
    parent = config_path.parent
...
def is_trusted(project_dir: str | Path) -> bool:
    """True when the project's CURRENT devcontainer.json carries a grant."""
...
def grant_trust(project_dir: str | Path) -> str:
...
class DevcontainerManager:
    ...
    async def up(self, project_dir: str | Path, *, rebuild: bool = False) -> DevcontainerInfo:
        ...
        """
        key = os.path.realpath(str(project_dir))
        cfg = find_devcontainer_config(key)
        if cfg is None:
    ...
    async def status(self, project_dir: str | Path) -> dict:
        ...
        out: dict = {
            ...
            "trusted": is_trusted(key) if cfg else False,
            "container_id": None,
            "running": False,
