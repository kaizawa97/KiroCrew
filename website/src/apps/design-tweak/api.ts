 ...
 *
 * This is the SAME idempotent call that adopts the slot — it already returns
 * both lists, so verifying delivery needs no new endpoint and no extra state.
 * Returns `null` when the lookup fails, which callers must treat as "unknown"
...
export async function readSlotTranscript(
  ...
  try {
    const r = await createChatSlot(name, title)
    return { messages: r?.messages, queue: r?.queue }
  } catch {
    return null
  }
}
