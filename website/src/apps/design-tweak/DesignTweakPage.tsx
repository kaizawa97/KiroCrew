...
import {
  ...
  createChatSlot, sendChatMessage,
...
function djb2Base36(str: string): string {
  let t = 0
  for (let i = 0; i < (str || '').length; i++) t = ((t << 5) - t + str.charCodeAt(i)) | 0
  return (t >>> 0).toString(36)
}
function slotKeyFor(path: string): string { return `${APP_SLOT_PREFIX}-${djb2Base36(path || '')}` }
...
function RequestGroup({
  req, open, onToggle, onSend, onResend, sending,
    ...
    <div className="mb-0.5">
      ...
      >
        ...
        {!done && !isDraft && (
          <div className={'flex items-center gap-0.5 ' + REVEAL_CLUSTER + ' -ml-2'}>
            ...
            {iconBtn(i18nT('apps.designTweak.requests.archive_to_history'), <Archive size={13} />, onArchive)}
            {iconBtn(i18nT('apps.designTweak.requests.delete_request'), <Trash2 size={13} />, onDelete)}
          </div>
        )}
      </Clickable>
      ...
      <FolderBody open={open}>
        <div className="border-l border-border mb-1 ml-3 pl-1 rounded-bl-md">
          ...
          {!done && !isDraft && needsDeliveryRetry(req) && comments.length > 0 && (
            <div>
              <div className="mx-3 mt-1 border-b border-border" />
              <button
...
export default function DesignTweak() {
  ...
  const ensureSlot = useCallback(async (path: string, label: string): Promise<string> => {
    ...
    try {
      ...
      if (!slot?.messages?.length) await sendChatMessage(SESSION_SEED(label, path), key)
  ...
  // the slot lookup returns the recent transcript plus the pending queue.
  //
  // Found -> acknowledge it and the request goes quiet, so the user never sees a
  // control for work that is already in hand. Not found -> leave it alone; the
  // rail offers a Send, which is now provably safe. Lookup failed -> change
  ...
  const verifyDelivery = useCallback(async (reqs: Request[]) => {
    ...
    const proj = projects.find((p) => p.id === previewId)
    const path = pending.find((r) => r.projectRoot)?.projectRoot || proj?.path || ''
    if (!path) return
    const transcript = await readSlotTranscript(path, proj?.name || FALLBACK_APP_LABEL)
    if (!transcript) return
    let healed = 0
    for (const r of pending) {
      if (deliveryVerdict(r, transcript) === 'delivered') {
                  ...
                  : !devOpen
                    ...
                    : (
                      <div className="flex items-center gap-1">
                        ...
                        >
                          {i18nT('apps.designTweak.devServer.cancel')}
                        </button>
                      </div>
                    )}
              </div>
