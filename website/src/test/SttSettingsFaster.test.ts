...
describe('provider labels', () => {
  it('labels every provider the backend can advertise', () => {
    // `_VALID_STT_PROVIDERS` in the config loader. An id absent here falls back to
    // the raw string, so the dropdown would read "faster".
    expect(Object.keys(PROVIDER_LABEL_KEY).sort()).toEqual(
      ['faster', 'mlx', 'transcribe', 'whisper'],
    )
  })
...
describe('the faster-whisper install copy', () => {
  it('exists and says ffmpeg is not needed', () => {
    // The one thing that makes this provider worth choosing on a machine where the
    // CLI toolchain is the hard part.
