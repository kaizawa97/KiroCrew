...
describe('sessionKey (resumable chat-session identity)', () => {
  it('namespaces by kind so the same number is not one conversation', () => {
    ...
    expect(sessionKey('pr', 7, R)).toBe('pr-octo-repo-7')
    expect(sessionKey('finding', 7, R)).toBe('finding-octo-repo-7')
  })
  ...
  it('sanitizes an id into a filename-safe key', () => {
    ...
    expect(sessionKey('finding', '../../etc/passwd', R)).toBe('finding-octo-repo-etc-passwd')
    expect(sessionKey('run', 'a b/c', R)).toBe('run-octo-repo-a-b-c')
  })

  it('never produces an empty key', () => {
    expect(sessionKey('run', '///', R)).toBe('run-octo-repo-unknown')
    expect(sessionKey('run', '', R)).toBe('run-octo-repo-unknown')
  })

  it('keeps a fingerprint intact', () => {
    expect(sessionKey('finding', 'deadbeef1234', R)).toBe('finding-octo-repo-deadbeef1234')
  })
})
