...
export default function ChatPage({ mode, embedded, embedMode, popout, noUrlSync }: { mode?: string; embedded?: boolean; embedMode?: 'chat' | 'sessions'; popout?: boolean; noUrlSync?: boolean } = {}) {
 ...
 // composer instead of auto-submitting. Auto-submitting would be a
  ...
  const devcontainerPromptKey = devcontainer?.has_config
    ? (devcontainer.config_path || devcontainer.project_dir)
    : ''
  ...
  const showDevcontainerTrust = !!devcontainer?.enabled
    ...
    && !devcontainer?.trusted
    && !devcontainerDismissed[devcontainerPromptKey]
  const trustDevcontainer = useCallback(async () => {
    if (!_slotProject) return
    await api.devcontainerTrust?.(_slotProject)
    await refetchDevcontainer()
  }, [_slotProject, refetchDevcontainer])
        ...
        {splitMode && splitFeatureEnabled ? (
          ...
          <div className="relative flex flex-col flex-1 min-h-0">
            ...
            <div className="relative">
              ...
              onDevcontainerUntrust={refetchDevcontainer}
              isMac={isMac}
              onDrop={handleDrop}
              dragOver={dragOver}
