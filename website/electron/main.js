...
const { stopGatewayGracefully: _stopGatewayGracefully, forceStopPort, classifyPortOwner, KIROCREW_PROC_RE } = require("./gateway-stop");
...
async function _probePortOwnerWin32(port) {
  try {
    ...
    if (pids === null) return "unknown"; // probe itself failed — never claim "none"
    if (!pids.length) return "none";
    for (const pid of pids) {
      const cmd = await _tasklistCommand(pid);
      if (KIROCREW_PROC_RE.test(cmd)) return "kirocrew";
    }
    return "foreign";
  } catch (e) {
...
async function forceStopGatewayPortWin32(port) {
  ...
  const pids = await _netstatListenPids(port);
  if (pids === null) {
    // The probe itself failed — we cannot prove the port is free, so we must
    // NOT report freed:true (that would let the caller respawn into EADDRINUSE).
    glog(`force-stop(win32): netstat probe failed on :${port} — cannot confirm state`);
    return { killed: 0, freed: false, survivors: [], foreignHolder: false, serviceHolder: false };
  }
  if (!pids.length) {
  ...
  // A failed verify probe cannot prove the port freed — treat as still-held.
  if (remaining === null) {
    glog(`force-stop(win32): verify netstat probe failed on :${port} — cannot confirm freed`);
    return { killed, freed: false, survivors: targets, foreignHolder: false, serviceHolder: false };
  }
 ...
 * Locale-safe: matches by column structure (local-addr:port + wildcard foreign
 * endpoint) rather than the state string, which is translated on non-English
 * Windows (e.g. "ABHÖREN", "ÉCOUTE").
 * Returns an array of PIDs, or `null` when the probe itself fails (netstat
 * error/timeout) — callers MUST distinguish that from an empty array (a real
 * "no listener") so a failed probe is never mistaken for a free port. */
  ...
  return new Promise((resolve) => {
    execFile("netstat", ["-ano", "-p", "TCP"], { timeout: 5000 }, (err, stdout) => {
      ...
      for (const line of String(stdout).split(/\r?\n/)) {
        ...
        // Expected columns: Proto LocalAddr ForeignAddr State PID
        if (parts.length < 5) continue;
        if (!/^TCP/i.test(parts[0])) continue;
        const localAddr = parts[1];
        const foreignAddr = parts[2];
        // A listening socket has the local port we want + wildcard foreign endpoint
        if (!portRe.test(localAddr)) continue;
        if (foreignAddr !== "0.0.0.0:0" && foreignAddr !== "[::]:0") continue;
...
/** Get the full command line for a PID via WMIC, falling back to PowerShell
 * Get-CimInstance if wmic is unavailable (deprecated/removed on some Win 11). */
  ...
  return new Promise((resolve) => {
    execFile("wmic", ["process", "where", `ProcessId=${pid}`, "get", "CommandLine", "/FORMAT:LIST"],
      { timeout: 5000 }, (err, stdout) => {
        ...
        // Fallback: PowerShell Get-CimInstance (works when wmic is absent)
        ...
        execFile("powershell", ["-NoProfile", "-NonInteractive", "-Command", ps],
          { timeout: 8000 }, (_psErr, psOut) => {
            resolve(String(psOut || "").trim());
          });
      });
  });
}
