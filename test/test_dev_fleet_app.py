...
def test_register_skills_creates_symlinks_for_bundled_skills(tmp_path, monkeypatch):
    ...
    # is_dir_link, not is_symlink: registration links with a directory junction
    ...
    for skill_name in expected_skills:
        ...
        assert platform_compat.is_dir_link(link), f"Namespaced link missing: {link}"
        assert link.resolve().is_dir()
        flat = skills_dir / skill_name
        assert platform_compat.is_dir_link(flat), f"Flat link missing: {flat}"
