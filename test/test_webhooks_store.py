...
class TestRunRing:
    ...
    def test_a_persisted_outcome_this_build_does_not_know_is_skipped(self, runs):
        """A row whose outcome is unknown must not reach a reader.

        ``record`` validates on write, but the file is also whatever is on disk
        after a hand-edit, a partial write, or a downgrade from a build with more
        outcomes. The dashboard indexes its label/badge table BY outcome, so an
        unrecognised value arrives as a missing entry and takes the whole page
        down with its error boundary. One bad row costs that row, not the page.
        """
        ...
        rows.append({**rows[0], "id": "run_bogus", "outcome": "bogus"})
