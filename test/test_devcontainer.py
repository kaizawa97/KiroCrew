...
M1 (spoofable pidfile kill target), and M3 (config swap between trust grant
and build).
"""
...
class TestConfigDigest:
    ...
    def test_digest_is_stable_for_identical_input(self, tmp_path: Path) -> None:
        cfg = _write_primary(tmp_path)
        (cfg.parent / "Dockerfile").write_text("FROM ubuntu:24.04\n")
    ...
    def test_symlinks_in_the_tree_are_ignored(self, tmp_path: Path) -> None:
        """A link inside .devcontainer/ must not pull outside bytes into the
        digest, or the trust hash would change under the user's feet whenever
        an unrelated host file did."""
...
class TestConfigReadHardening:
    ...
    def test_symlink_leaf_is_treated_as_absent(self, tmp_path: Path) -> None:
        project = tmp_path / "proj"
        project.mkdir()
    ...
    def test_symlink_root_layout_leaf_is_treated_as_absent(self, tmp_path: Path) -> None:
        project = tmp_path / "proj"
        project.mkdir()
        secret = tmp_path / "credentials"
    ...
    def _escaping_project(self, tmp_path: Path) -> tuple[Path, Path]:
        ...
        outside.mkdir()
        (outside / "devcontainer.json").write_text('{"image": "attacker/img:latest"}')
        (project / ".devcontainer").symlink_to(outside, target_is_directory=True)
        return project, project / ".devcontainer" / "devcontainer.json"

    def test_read_refuses_a_config_escaping_the_project(self, tmp_path: Path) -> None:
    ...
    def test_preview_surfaces_the_escape_refusal(self, tmp_path: Path, trust_home: Path) -> None:
    ...
    def test_read_refuses_a_non_regular_file(self, tmp_path: Path) -> None:
        """fstat gate: a directory opens fine on Linux, S_ISREG rejects it."""
        project = tmp_path / "proj"
        project.mkdir()
        as_dir = project / ".devcontainer" / "devcontainer.json"
        as_dir.mkdir(parents=True)
        with pytest.raises(devc.DevcontainerError, match="not a regular file"):
...
class TestTrustStore:
    ...
    def test_grant_records_digest_and_config_path(self, tmp_path: Path, trust_home: Path) -> None:
        ...
        data = json.loads((trust_home / "devcontainers" / "trust.json").read_text())
    ...
    def test_trust_invalidated_when_config_bytes_change(
        ...
        cfg.write_text(json.dumps({"name": "kirocrew-dev", "image": "evil:latest"}))
        assert devc.is_trusted(project) is False

        # Restoring the exact original bytes restores the grant: trust binds to
        # content, not to an edit counter.
        cfg.write_text(_SAMPLE_CONFIG)
        assert devc.is_trusted(project) is True
...
class TestUpTrustGate:
    ...
    async def test_edited_config_invalidates_trust_before_spawn(
        ...
        cfg = _write_primary(project)
        devc.grant_trust(project)
        cfg.write_text('{"image": "attacker/img:latest"}')
...
class TestUpPostBuildDigestReverification:
    ...
    async def test_swap_with_no_container_id_still_refuses(
        ...
        proc = _FakeProc(
            stdout=(record + "\n").encode(),
            on_communicate=lambda: cfg.write_text('{"image": "evil"}'),
        )
...
class TestResolveProject:
    ...
    @pytest.mark.asyncio
    async def test_accepts_a_realpath_match_through_a_symlink(self, tmp_path: Path) -> None:
