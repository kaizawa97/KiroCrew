...
class _StubStore:
    ...
    def resolve_meeting_dir(
        self, meeting_id: str, root: Optional[Path] = None
    ) -> Optional[Path]:
...
class TestRecordingStorageWiring:
    ...
    @pytest.mark.asyncio
    async def test_start_without_a_meeting_id_still_works(
    ...
    async def test_rejects_a_malformed_meeting_id(
        self, patched_guards: None, bad: object
    ) -> None:
