...
``MARKER_CLOSERS`` is therefore shared by both regexes AND by
:func:`split_trailing_protocol_suffix`, so a marker that one accepts can never be
one the others consider unfinished. These tests lock that agreement in, and lock
in that widening the closer did NOT widen anything else.
"""
